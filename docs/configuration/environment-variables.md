...
| Dashboard URL (sample legacy) | `PATCHWORK_DASHBOARD_URL` | No (currently unused by application code) | Present in `.env.sample` for deployment context, but not consumed by the current code paths. Treat as informational unless a local deployment script depends on it. |
