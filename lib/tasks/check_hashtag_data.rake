namespace :check_hashtag_data do
  desc "Check for unexpected tag follows for community boost bots"
  task check_unexpected_tag_follows: :environment do
    puts "--- Checking for missing tag follows ---"
    count_missing = 0
    ...
    results_missing.each do |record|
      unless record.is_followed
        puts "🚨 Missing follow found -> Community: '#{record.community_name}' | Bot ID: #{record.boost_bot_username} | Tag: #{record.community_tag}"
        count_missing += 1
      end
    end
    ...
    puts "--- Checking for unexpected tag follows ---"
    count_unexpected = 0
    ...
    results_unexpected.each do |record|
      if record.is_unexpected
        puts "⚠️ Unexpected follow found -> Community: '#{record.community_name}' | Bot ID: #{record.bot_account_id} | Tag: #{record.tag_name}"
        count_unexpected += 1
      end
    end
